...
{
    private readonly SemaphoreSlim _connectionManagementSemaphore = new(1);

    private readonly object _interruptionCauseLock = new();
    private readonly ILogger<RfbConnection> _logger;

    private readonly object _outputHandlerLock = new();

    private readonly CancellationTokenSource _reconnectCts = new();

    private readonly object _renderTargetLock = new();
    ...
    public ConnectionState ConnectionState
    {
        ...
        get
            => (ConnectionState)Interlocked.CompareExchange(ref Unsafe.As<ConnectionState, int>(ref _connectionState),
                0, 0);
        private set
        {
            if (Interlocked.Exchange(ref Unsafe.As<ConnectionState, int>(ref _connectionState), (int)value)
                != (int)value)
            {
    ...
    /// </summary>
    /// <remarks>
    ///     To cancel the connection establishment, use the <see cref="CancellationToken" /> passed to
    ///     <see cref="StartAsync" /> instead.
    /// </remarks>
    ...
    private void OnRunningConnectionFailed(Exception exception)
    {
        ...
        if (Interlocked.CompareExchange(ref Unsafe.As<ConnectionState, int>(ref _connectionState),
                (int)ConnectionState.Interrupted, (int)ConnectionState.Connected) != (int)ConnectionState.Connected)
        {
            return;
        }
    ...
    private async Task ReconnectAsync()
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                if (Parameters.MaxReconnectAttempts != ConnectParameters.InfiniteReconnects
                    && failedAttempts >= Parameters.MaxReconnectAttempts)
                {
                    // Giving up.
                ...
                catch
                {
                    ...
                    _logger.LogWarning("Reconnect attempt {attempt} to {endpoint} failed.", failedAttempts,
                        Parameters.TransportParameters);
                    CleanupPreviousConnection();
