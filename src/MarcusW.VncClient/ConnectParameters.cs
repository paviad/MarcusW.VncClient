...
public sealed class ConnectParameters : FreezableParametersObject
{
    ...
    /// <summary>
    ///     Gets or sets the <see cref="IAuthenticationHandler" /> implementation that provides information for authenticating
    ///     against the server.
    /// </summary>
    ...
    /// <summary>
    ///     Gets or sets the target where received frames should be rendered to, in case you want to set the target from the
    ///     start on.
    /// </summary>
    ...
    public override void Validate()
    {
        if (TransportParameters == null)
        {
            throw new ConnectParametersValidationException(
                $"{nameof(TransportParameters)} parameter must not be null.");
        }
        ...
        if (JpegQualityLevel is < 0 or > 100)
        {
            throw new ConnectParametersValidationException(
                $"{nameof(JpegQualityLevel)} parameter is not a valid percentage.");
        }
    ...
    /// <inheritdoc />
    protected override IEnumerable<FreezableParametersObject?> GetDescendants() => [TransportParameters];
}
