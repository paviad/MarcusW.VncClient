...
public readonly struct PixelFormat : IEquatable<PixelFormat>
{
    ...
    /// </summary>
    public static readonly PixelFormat Unknown = new("Unknown", 0, 0, false, false, false, 0, 0, 0, 0, 0, 0, 0, 0);

    /// <summary>
    ///     A very basic RGBA pixel format.
    /// </summary>
    public static readonly PixelFormat Plain = new("Plain RGBA", 32, 32, false, true, true, 255, 255, 255, 255, 24, 16,
        8, 0);
    ...
    /// <summary>
    ///     Gets whether the pixel value is composed from the color values (True), or if the color values serve as indices into
    ///     a color map (False).
    /// </summary>
    ...
    /// <param name="depth">
    ///     The number of useful bits in the pixel value. Must be greater than or equal to
    ///     <paramref name="bitsPerPixel" />.
    /// </param>
    /// <param name="bigEndian">True if multi-byte pixels are interpreted as big endian, otherwise false.</param>
    /// <param name="trueColor">
    ///     True if the pixel value is composed from the color values, or false if the color values serve
    ///     as indices into a color map.
    /// </param>
    ...
    public PixelFormat(string name, byte bitsPerPixel, byte depth, bool bigEndian, bool trueColor, bool hasAlpha,
        ushort redMax, ushort greenMax, ushort blueMax, ushort alphaMax, byte redShift, byte greenShift, byte blueShift,
        byte alphaShift)
    {
        Name = name;
    ...
    public bool IsBinaryCompatibleTo(PixelFormat other, bool ignoreAlpha = false)
    {
        ...
        if (RedMax != other.RedMax || GreenMax != other.GreenMax || BlueMax != other.BlueMax
            || RedShift != other.RedShift || GreenShift != other.GreenShift || BlueShift != other.BlueShift)
        {
            return false;
        }
