...
{
    /// <summary>
    ///     Reads <paramref name="sourceLength" /> bytes from the <paramref name="source" /> stream and returns a stream for
    ///     inflating them.
    /// </summary>
    /// <remarks>
    ///     For optimization reasons, this method must not be called again before the reading from the returned stream is
    ///     over.
    /// </remarks>
    ...
    Stream ReadAndInflate(Stream source, int sourceLength, int zlibStreamId = -1,
