...
public interface IRfbMessageSender : IBackgroundThread
{
    ...
    /// <remarks>
    ///     Please ensure the outgoing message type is marked as being supported by both sides before sending it. See
    ///     <see cref="RfbConnection.UsedMessageTypes" />.
    /// </remarks>
    void EnqueueMessage<TMessageType>(IOutgoingMessage<TMessageType> message,
    ...
    ///     <see cref="RfbConnection.UsedMessageTypes" />.
    /// </remarks>
    void SendMessageAndWait<TMessageType>(IOutgoingMessage<TMessageType> message,
        CancellationToken cancellationToken = default) where TMessageType : class, IOutgoingMessageType;

    /// <summary>
    ///     Adds the <paramref name="message" /> to the send queue and returns a <see cref="Task" /> that completes when the
    ///     message was sent.
    /// </summary>
    /// <param name="message">The message to send.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    ...
    ///     <see cref="RfbConnection.UsedMessageTypes" />.
    /// </remarks>
    Task SendMessageAndWaitAsync<TMessageType>(IOutgoingMessage<TMessageType> message,
        CancellationToken cancellationToken = default) where TMessageType : class, IOutgoingMessageType;
