...
{
    /// <summary>
    ///     Reads pixel data from <paramref name="pixelPtr" />, converts it to <paramref name="targetFormat" /> and writes it
    ...
    public static unsafe void WritePixel(byte* pixelPtr, in PixelFormat pixelFormat, byte* targetPtr,
        in PixelFormat targetFormat)
    {
        ...
        if (pixelFormat.BitsPerPixel > 32 || targetFormat.BitsPerPixel > 32)
        {
            throw new InvalidOperationException(
                "This conversion algorithm doesn't support pixel formats with more than 32bpp.");
        }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private static void CopyChannelValue(uint srcValue, ref uint dstValue, ushort srcMax, byte srcShift, ushort dstMax,
        byte dstShift)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private static unsafe bool WritePixelFastPath(byte* pixelPtr, in PixelFormat pixelFormat, byte* targetPtr,
        in PixelFormat targetFormat)
    {
            ...
            else
            {
                ...
                if (pixelFormat.IsBinaryCompatibleTo(targetFormat, true))
                {
                    ...
                    if (targetFormat.HasAlpha)
                    {
                        ...
                        {
                            case 32:
                                Unsafe.Write(targetPtr,
                                    Unsafe.AsRef<uint>(targetPtr)
                                    | (uint)(targetFormat.AlphaMax << targetFormat.AlphaShift));
                                break;
                            case 16:
                                Unsafe.Write(targetPtr,
                                    Unsafe.AsRef<ushort>(targetPtr)
                                    | (ushort)(targetFormat.AlphaMax << targetFormat.AlphaShift));
                                break;
                            case 8:
                                Unsafe.Write(targetPtr,
                                    Unsafe.AsRef<byte>(targetPtr)
                                    | (byte)(targetFormat.AlphaMax << targetFormat.AlphaShift));
                                break;
                            default:
                                Debug.Fail(
                                    $"Fast path optimization for pixel conversions might not work correctly for strange bpp-values like {targetFormat.BitsPerPixel}.");
                                break;
                        }
                    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private static unsafe void WritePixelGenericPath(byte* pixelPtr, in PixelFormat pixelFormat, byte* targetPtr,
        in PixelFormat targetFormat)
    {
        ...
        switch (pixelFormat.BitsPerPixel)
        {
            case 32:
            case 24
                : // Used for ZRLE/Tight compressed pixels which consist of only 3 bytes. The fourth read byte will be ignored then.
            ...
            default:
                throw new InvalidOperationException(
                    $"Generic pixel conversion doesn't support strange bpp-values like {pixelFormat.BitsPerPixel}.");
        }
        ...
        CopyChannelValue(pixelValue, ref targetValue, pixelFormat.RedMax, pixelFormat.RedShift, targetFormat.RedMax,
            targetFormat.RedShift);
        CopyChannelValue(pixelValue, ref targetValue, pixelFormat.GreenMax, pixelFormat.GreenShift,
            targetFormat.GreenMax, targetFormat.GreenShift);
        CopyChannelValue(pixelValue, ref targetValue, pixelFormat.BlueMax, pixelFormat.BlueShift, targetFormat.BlueMax,
            targetFormat.BlueShift);

        // Copy or fake the alpha channel
        if (pixelFormat.HasAlpha && targetFormat.HasAlpha)
        {
            CopyChannelValue(pixelValue, ref targetValue, pixelFormat.AlphaMax, pixelFormat.AlphaShift,
                targetFormat.AlphaMax, targetFormat.AlphaShift);
        }
        ...
        switch (targetFormat.BitsPerPixel)
        {
            ...
            default:
                throw new InvalidOperationException(
                    $"Generic pixel conversion doesn't support strange bpp-values like {targetFormat.BitsPerPixel}.");
        }
    }
}
