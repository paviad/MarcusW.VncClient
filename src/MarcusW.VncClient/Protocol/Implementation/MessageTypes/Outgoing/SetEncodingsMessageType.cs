...
public class SetEncodingsMessageType : IOutgoingMessageType
{
    ...
    {
        if (message is not SetEncodingsMessage setEncodingsMessage)
        {
...
public class SetEncodingsMessage(IImmutableSet<IEncodingType> supportedEncodingTypes)
    : IOutgoingMessage<SetEncodingsMessageType>
{
    /// <summary>
    ///     Gets the encoding types collection that will be reported to the server.
    /// </summary>
    public IImmutableSet<IEncodingType> SupportedEncodingTypes { get; } = supportedEncodingTypes;

    /// <inheritdoc />
    public string GetParametersOverview()
        => $"SupportedEncodingTypes: {string.Join(", ", SupportedEncodingTypes.Select(et => et.Name))}";
}
