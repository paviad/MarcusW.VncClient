...
public class ExtendedDesktopSizeEncodingType(RfbConnectionContext context) : PseudoEncodingType
{
    private readonly ILogger<ExtendedDesktopSizeEncodingType> _logger =
        context.Connection.LoggerFactory.CreateLogger<ExtendedDesktopSizeEncodingType>();
    ...
    public override void ReadPseudoEncoding(Stream transportStream, Rectangle rectangle)
    {
        ...
        if (!_state.ServerSupportsExtendedDesktopSize)
        {
            ...
            _state.EnsureMessageTypeIsMarkedAsUsed<IOutgoingMessageType>(null,
                (byte)WellKnownOutgoingMessageType.SetDesktopSize);
        ...
        {
            _logger.LogWarning("Server responded to a previously requested desktop size change with an error: "
                + statusCode switch {
        ...
        Span<byte> headerSpan = readBuffer[..4];
        ...
        for (var i = 0; i < numberOfScreens; i++)
        {
            ...
            screens[i] = new(id, new(x, y, width, height), flags);
        }
        ...
        if (screens.Select(s => s.Id).Distinct().Count() != numberOfScreens)
        {
            throw new UnexpectedDataException(
                "At least two of the received framebuffer screens have conflicting IDs. This is not allowed.");
        }
        ...
        {
            throw new UnexpectedDataException(
                "At least one of the received framebuffer screens lies (partially) outside of the framebuffer area.");
        }
        ...
        _logger.LogDebug(
            "Remote framebuffer size updated to {newSize} with a {screenCount}-screen layout: {layout}. Reason: {changeReason}",
            newSize, numberOfScreens, string.Join(", ", screens), changeReason);
        ...
        if (_state is { ServerSupportsContinuousUpdates: true, ContinuousUpdatesEnabled: true })
        {
