...
public class ZrleEncodingType(RfbConnectionContext context) : FrameEncodingType
{
    ...
    /// <inheritdoc />
    public override Color VisualizationColor => new(0, 255, 0);
        ...
        in Rectangle rectangle, in Size remoteFramebufferSize, in PixelFormat remoteFramebufferFormat)
    {
        ...
        for (int tileY = rectangle.Position.Y; tileY < rectBottomY; tileY += TileSize)
        {
            ...
            for (int tileX = rectangle.Position.X; tileX < rectRightX; tileX += TileSize)
            {
                ...
                // Read tile
                ReadTile(inflateStream, targetFramebuffer, new(tileX, tileY, tileWidth, tileHeight), cPixelFormat);
            }
        }
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static PixelFormat GetCPixelFormat(in PixelFormat remoteFramebufferFormat)
    {
        // See https://github.com/TigerVNC/tigervnc/blob/d8bbbeb3b37c713a72a113f7ef78741e15cc4a4d/common/rfb/ZRLEDecoder.cxx#L84

        if (remoteFramebufferFormat is { TrueColor: true, BitsPerPixel: 32 } and { Depth: <= 24, HasAlpha: false })
        {
            ...
            var maxPixel = (uint)((remoteFramebufferFormat.RedMax << remoteFramebufferFormat.RedShift)
                | (remoteFramebufferFormat.GreenMax << remoteFramebufferFormat.GreenShift)
                | (remoteFramebufferFormat.BlueMax << remoteFramebufferFormat.BlueShift));
            ...
            if ((fitsInLs3Bytes && remoteFramebufferFormat.LittleEndian)
                || (fitsInMs3Bytes && remoteFramebufferFormat.BigEndian))
            {
                ...
                return new(remoteFramebufferFormat.Name, 24, remoteFramebufferFormat.Depth,
                    remoteFramebufferFormat.BigEndian, true, false, remoteFramebufferFormat.RedMax,
                    remoteFramebufferFormat.GreenMax, remoteFramebufferFormat.BlueMax, 0,
                    remoteFramebufferFormat.RedShift, remoteFramebufferFormat.GreenShift,
                    remoteFramebufferFormat.BlueShift, 0);
            }
            ...
            if ((fitsInLs3Bytes && remoteFramebufferFormat.BigEndian)
                || (fitsInMs3Bytes && remoteFramebufferFormat.LittleEndian))
            {
                ...
                return new(remoteFramebufferFormat.Name, 24, remoteFramebufferFormat.Depth,
                    remoteFramebufferFormat.BigEndian, true, false, remoteFramebufferFormat.RedMax,
                    remoteFramebufferFormat.GreenMax, remoteFramebufferFormat.BlueMax, 0,
                    (byte)(remoteFramebufferFormat.RedShift + shiftOffset),
                    (byte)(remoteFramebufferFormat.GreenShift + shiftOffset),
    ...
    private void ReadPackedPaletteTile(Stream stream, bool hasTargetFramebuffer,
        ref FramebufferCursor framebufferCursor, in Rectangle tile, in PixelFormat cPixelFormat, int paletteSize)
    {
        ...
        int packedPixelBytes = bitsPerPackedPixel switch {
            ...
            _ => throw new ArgumentOutOfRangeException(nameof(paletteSize)),
        };
        ...
        Span<byte> buffer = _tileBuffer.AsSpan()[..totalBytesToRead];
        stream.ReadAll(buffer);

        Span<byte> palette = buffer[..paletteBytes];
                    ...
                    for (var j = 0; j < tile.Size.Width; j++)
                    {
                        ...
                        if (paletteIndex >= paletteBytes)
                        {
                            throw new UnexpectedDataException(
                                $"Received invalid packed palette index of {paletteIndex} for a palette of {paletteBytes} bytes.");
                        }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private void ReadPaletteRleTile(Stream stream, bool hasTargetFramebuffer, ref FramebufferCursor framebufferCursor,
        in Rectangle tile, in PixelFormat cPixelFormat, int paletteSize)
    {
        byte bytesPerPixel = cPixelFormat.BytesPerPixel;
        int paletteBytes = paletteSize * bytesPerPixel;

        // Read palette bytes
        Span<byte> palette = _tileBuffer.AsSpan()[..paletteBytes];
                ...
                while (pixelsRemaining > 0)
                {
                    ...
                    if (stream.Read(readBuffer) == 0)
                    {
                        throw new UnexpectedEndOfStreamException(
                            "Stream reached its end while reading ZRLE RLE palette index.");
                    }
                    ...
                    if ((paletteIndexByte & 128) != 0)
                    {
                        ...
                        do
                        {
                            ...
                            {
                                throw new UnexpectedEndOfStreamException(
                                    "Stream reached its end while reading ZRLE RLE run-length.");
                            }
                    ...
                    if (hasTargetFramebuffer)
                    {
                        ...
                        {
                            throw new UnexpectedDataException(
                                $"Received invalid RLE palette index of {paletteIndex} for a palette of {paletteBytes} bytes.");
                        }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private void ReadRawTile(Stream stream, bool hasTargetFramebuffer, ref FramebufferCursor framebufferCursor,
        in Rectangle tile, in PixelFormat cPixelFormat)
    {
        // Calculate how many bytes we're going to receive
        byte bytesPerPixel = cPixelFormat.BytesPerPixel;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private void ReadRleTile(Stream stream, bool hasTargetFramebuffer, ref FramebufferCursor framebufferCursor,
        in Rectangle tile, in PixelFormat cPixelFormat)
    {
        ...
        unsafe
        {
            fixed (byte* bufferPtr = buffer)
            {
                ...
                while (pixelsRemaining > 0)
                {
                    ...
                    stream.ReadAll(buffer[..(bytesPerPixel + 1)]);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private void ReadSolidTile(Stream stream, bool hasTargetFramebuffer, ref FramebufferCursor framebufferCursor,
        in Rectangle tile, in PixelFormat cPixelFormat)
    {
        // Read a single color value
        Span<byte> buffer = _tileBuffer.AsSpan()[..cPixelFormat.BytesPerPixel];
        stream.ReadAll(buffer);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private void ReadTile(Stream stream, IFramebufferReference? targetFramebuffer, in Rectangle tile,
        in PixelFormat cPixelFormat)
    {
        ...
        FramebufferCursor framebufferCursor =
            hasTargetFramebuffer ? new FramebufferCursor(targetFramebuffer!, tile) : default;
        ...
        if (!isRunLengthEncoded)
        {
            switch (paletteSize)
            {
                ...
                // Packed palette
                case >= 2 and <= 16:
                    ReadPackedPaletteTile(stream, hasTargetFramebuffer, ref framebufferCursor, tile, cPixelFormat,
                        paletteSize);
                    break;
        ...
        else
        {
            switch (paletteSize)
            {
                ...
                case >= 2 and <= 127:
                    ReadPaletteRleTile(stream, hasTargetFramebuffer, ref framebufferCursor, tile, cPixelFormat,
                        paletteSize);
                    break;
