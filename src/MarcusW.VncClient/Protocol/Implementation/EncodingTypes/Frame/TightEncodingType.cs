...
public class TightEncodingType(RfbConnectionContext context) : FrameEncodingType
{
    /// <inheritdoc />
    ...
    /// <inheritdoc />
    public override Color VisualizationColor => new(0, 255, 255);
        ...
        in Rectangle rectangle, in Size remoteFramebufferSize, in PixelFormat remoteFramebufferFormat)
    {
        ...
        IZLibInflater zlibInflater = context.ZLibInflater;
        ...
        FramebufferCursor framebufferCursor =
            hasTargetFramebuffer ? new FramebufferCursor(targetFramebuffer!, rectangle) : default;
        ...
        if ((compressionControl & 128) == 0) // Basic compression
        {
            ...
            ReadBasicCompressedRectangle(transportStream, hasTargetFramebuffer, ref framebufferCursor, rectangle,
                tPixelFormat, zlibStreamId, readFilterId);
        }
        else if ((compressionControl & 16) == 0) // Fill compression
        {
            ReadFillCompressedRectangle(transportStream, hasTargetFramebuffer, ref framebufferCursor, rectangle,
                tPixelFormat);
        }
        else // Jpeg compression
        {
            if (remoteFramebufferFormat.BitsPerPixel != 16 && remoteFramebufferFormat.BitsPerPixel != 32)
            {
                throw new UnexpectedDataException(
                    "Tight JPEG compression is not supported for bpp values other than 16 and 32.");
            }
    ...
    private static PixelFormat GetTPixelFormat(in PixelFormat remoteFramebufferFormat)
    {
        if (remoteFramebufferFormat.TrueColor && remoteFramebufferFormat is { BitsPerPixel: 32, Depth: 24 } and
                { HasAlpha: false, RedMax: 255 } and { GreenMax: 255, BlueMax: 255 })
        {
            // Received as R,G,B --> memory(LE): RGB0 (0x0BGR)
            return new(remoteFramebufferFormat.Name, 24, 24, false, true, false, 255, 255, 255, 0, 0, 8, 16, 0);
        }
    ...
    private Stream GetBasicCompressedPixelDataStream(Stream baseStream, int expectedLength, int zlibStreamId)
    {
        ...
        Debug.Assert(context.ZLibInflater != null, "_context.ZLibInflater != null");
        return context.ZLibInflater.ReadAndInflate(baseStream, zlibDataLength, zlibStreamId);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private void ReadBasicCompressedCopyFilterRectangle(Stream stream, bool hasTargetFramebuffer,
        ref FramebufferCursor framebufferCursor, in Rectangle rectangle, in PixelFormat tPixelFormat, int zlibStreamId)
    {
        ...
        Span<byte> pixelsSpan = pixelsBuffer.AsSpan()[..pixelsDataLength];
        try
        {
            // Read pixel data
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private void ReadBasicCompressedPaletteFilterRectangle(Stream stream, bool hasTargetFramebuffer,
        ref FramebufferCursor framebufferCursor, in Rectangle rectangle, in PixelFormat tPixelFormat, int zlibStreamId)
    {
        int bytesPerPixel = tPixelFormat.BytesPerPixel;
        ...
        if (stream.Read(paletteSizeBuffer) == 0)
        {
            throw new UnexpectedEndOfStreamException(
                "Stream reached its end while reading Tight basic compression palette size.");
        }
        ...
        int pixelsDataLength = isSingleBitEncoded
            ? (rectangle.Size.Width + 7) / 8 * rectangle.Size.Height
            : rectangle.Size.Width * rectangle.Size.Height;
        byte[] buffer = ArrayPool<byte>.Shared.Rent(paletteDataLength + pixelsDataLength);
        Span<byte> bufferSpan = buffer.AsSpan();
        Span<byte> paletteSpan = bufferSpan[..paletteDataLength];
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private void ReadBasicCompressedRectangle(Stream stream, bool hasTargetFramebuffer,
        ref FramebufferCursor framebufferCursor, in Rectangle rectangle, in PixelFormat tPixelFormat, int zlibStreamId,
        bool readFilterId)
    {
        ...
        if (readFilterId)
        {
            ...
            {
                throw new UnexpectedEndOfStreamException(
                    "Stream reached its end while reading Tight basic compression filter id.");
            }
        ...
        switch (filterId)
        {
            case 0: // CopyFilter (Raw)
                ReadBasicCompressedCopyFilterRectangle(stream, hasTargetFramebuffer, ref framebufferCursor, rectangle,
                    tPixelFormat, zlibStreamId);
                break;

            case 1: // PaletteFilter
                ReadBasicCompressedPaletteFilterRectangle(stream, hasTargetFramebuffer, ref framebufferCursor,
                    rectangle, tPixelFormat, zlibStreamId);
                break;

            case 2: // GradientFilter
                // TODO: Implement Tight gradient filter
                throw new UnsupportedProtocolFeatureException(
                    "Tight basic compression gradient filters are not yet supported.");
    ...
    private uint ReadCompactNumber(Stream stream)
    {
        ...
        while (true)
        {
            ...
            {
                throw new UnexpectedEndOfStreamException(
                    "Stream reached its end while reading a compact number representation.");
            }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private void ReadFillCompressedRectangle(Stream stream, bool hasTargetFramebuffer,
        ref FramebufferCursor framebufferCursor, in Rectangle rectangle, in PixelFormat tPixelFormat)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private void ReadJpegCompressedRectangle(Stream stream, bool hasTargetFramebuffer,
        ref FramebufferCursor framebufferCursor, in Rectangle rectangle)
    {
        Debug.Assert(context.JpegDecoder != null, "_context.JpegDecoder != null");
        IJpegDecoder jpegDecoder = context.JpegDecoder;
        ...
        byte[] pixelsBuffer = ArrayPool<byte>.Shared.Rent(pixelsDataLength);
        Span<byte> jpegSpan = jpegBuffer.AsSpan()[..jpegDataLength];
        Span<byte> pixelsSpan = pixelsBuffer.AsSpan()[..pixelsDataLength];
        try
        {
            ...
            jpegDecoder.DecodeJpegTo32Bit(jpegSpan, pixelsSpan, rectangle.Size.Width, rectangle.Size.Height,
                framebufferCursor.FramebufferFormat, out PixelFormat pixelFormat);
