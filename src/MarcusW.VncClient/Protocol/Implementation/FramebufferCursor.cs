...
public unsafe struct FramebufferCursor : IEquatable<FramebufferCursor>
{
    ...
    public FramebufferCursor(IFramebufferReference framebufferReference, in Rectangle rectangle) : this(
        (byte*)framebufferReference.Address, framebufferReference.Format, framebufferReference.Size, rectangle) { }
    ...
    /// <param name="rectangle">The target rectangle.</param>
    public FramebufferCursor(byte* framebufferPtr, in PixelFormat framebufferFormat, in Size framebufferSize,
        in Rectangle rectangle)
    {
        ...
        {
            throw new ArgumentException("Given rectangle lies (partially) outside of the framebuffer area.",
                nameof(rectangle));
        }
    ...
    public readonly bool GetEndReached() => _currentY == LastY && _currentX == LastX;
    ...
    public void MoveForwardInLine(int count)
    {
        ...
        if (_currentX + count > LastX)
        {
            throw new RfbProtocolException(
                $"Moving the framebuffer cursor {count} pixels forward in line would exceed the line end.");
        }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public readonly void SetPixel(Color color)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public readonly void SetPixel(byte* pixelData, in PixelFormat pixelFormat)
    {
        PixelConversions.WritePixel(pixelData, pixelFormat, _positionPtr, FramebufferFormat);
    }

    /// <summary>
    ///     Sets <paramref name="numPixels" /> pixels all to the same color value and advances the cursor accordingly.
    /// </summary>
    /// <param name="color">The desired color.</param>
    ...
    /// <summary>
    ///     Sets <paramref name="numPixels" /> pixels and advances the cursor accordingly.
    /// </summary>
    /// <param name="pixelData">A pointer to a memory location where the data for <paramref name="numPixels" /> is stored.</param>
    /// <param name="pixelFormat">The format of the pixel data.</param>
    /// <param name="numPixels">The number of pixels to set.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public void SetPixels(byte* pixelData, in PixelFormat pixelFormat, int numPixels)
    {
        ...
        if (pixelFormat.IsBinaryCompatibleTo(FramebufferFormat))
        {
            ...
            while (pixelsRemaining > 0)
            {
                ...
                {
                    throw new RfbProtocolException(
                        "There are pixels remaining in the buffer but the cursor already reached the end of the rectangle.");
                }
    ...
    public void CopyAllFrom(ref FramebufferCursor otherCursor)
    {
        ...
        if (otherRectangle.Size != Rectangle.Size || otherCursor.BytesPerPixel != BytesPerPixel
            || otherCursor.FramebufferFormat != FramebufferFormat)
        {
            throw new InvalidOperationException("The other cursor is not equal enough to attempt a full copy.");
        }

        if (_currentX != FirstX || otherCursor._currentX != otherCursor.FirstX || _currentY != Rectangle.Position.Y
            || otherCursor._currentY != otherRectangle.Position.Y)
        {
    ...
    /// <inheritdoc />
    public readonly bool Equals(FramebufferCursor other)
        => FramebufferFormat == other.FramebufferFormat && FirstX == other.FirstX && LastX == other.LastX
            && LastY == other.LastY && LineBreakBytes == other.LineBreakBytes;

    /// <inheritdoc />
    public override readonly bool Equals(object? obj) => obj is FramebufferCursor other && Equals(other);

    /// <inheritdoc />
    public override readonly int GetHashCode()
        => HashCode.Combine(FramebufferFormat, FirstX, LastX, LastY, LineBreakBytes);
}
