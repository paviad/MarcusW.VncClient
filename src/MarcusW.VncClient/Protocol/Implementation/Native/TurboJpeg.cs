...
/// </summary>
[PublicAPI]
public static partial class TurboJpeg
{
    // *** CREDITS ***
    ...
    ///     image that will fit within the desired width. If <paramref name="width" /> is set to 0, then only the height will
    ///     be considered when determining the scaled image size.
    /// </param>
    /// <param name="pitch">
    ///     Bytes per line in the destination image.
    ///     Normally, this is <c>scaledWidth * tjPixelSize[pixelFormat]</c> if the decompressed image is unpadded, else
    ///     <c>TJPAD(scaledWidth * tjPixelSize[pixelFormat])</c> if each
    ...
    ///     JPEG decompressor to generate the largest possible
    ///     image that will fit within the desired height. If <paramref name="height" /> is set to 0, then only the width will
    ///     be considered when determining the scaled image size.
    /// </param>
        ...
        int height, int pixelFormat, int flags)
    {
        return IntPtr.Size switch {
            ...
            var _ => throw new InvalidOperationException("Unsupported plattform architecture."),
        };
    }
    ...
    /// <param name="subsampling">
    ///     An integer variable that will receive the level of chrominance subsampling used when the JPEG
    ///     image was compressed.
    /// </param>
    /// <param name="colorspace">
    ///     An integer variable that will receive one of the JPEG colorspace constants, indicating the
    ///     colorspace of the JPEG image.
    /// </param>
    /// <returns>0 if successful, or -1 if an error occurred (see <see cref="DestroyInstance" />).</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    {
        return IntPtr.Size switch {
            4 => DecompressHeader_x86(handle, jpegBuf, (uint)jpegSize, out width, out height, out subsampling,
                out colorspace),
            8 => DecompressHeader_x64(handle, jpegBuf, jpegSize, out width, out height, out subsampling,
                out colorspace),
    ...
    /// <returns>0 if successful, or -1 if an error occurred (see <see cref="GetLastError" />).</returns>
    [LibraryImport(LibraryName, EntryPoint = "tjDestroy")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial int DestroyInstance(IntPtr handle);

    /// <summary>
    ///     Returns a descriptive error message explaining why the last command failed.
    /// </summary>
    /// <returns>An error message.</returns>
    [LibraryImport(LibraryName, EntryPoint = "tjGetErrorStr")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    ...
    /// </summary>
    /// <returns>
    ///     A handle to the newly created instance, or <see langword="null" /> if an error occurred (see
    ///     <see cref="GetLastError" />).
    /// </returns>
    [LibraryImport(LibraryName, EntryPoint = "tjInitDecompress")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial IntPtr InitDecompressorInstance();

    [LibraryImport(LibraryName, EntryPoint = "tjDecompress2")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial int Decompress_x64(IntPtr handle, IntPtr jpegBuf, ulong jpegSize, IntPtr dstBuf, int width,
        int pitch, int height, int pixelFormat, int flags);

    [LibraryImport(LibraryName, EntryPoint = "tjDecompress2")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial int Decompress_x86(IntPtr handle, IntPtr jpegBuf, uint jpegSize, IntPtr dstBuf, int width,
        int pitch, int height, int pixelFormat, int flags);

    [LibraryImport(LibraryName, EntryPoint = "tjDecompressHeader3")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial int DecompressHeader_x64(IntPtr handle, IntPtr jpegBuf, ulong jpegSize, out int width,
        out int height, out int jpegSubsamp, out int jpegColorspace);

    [LibraryImport(LibraryName, EntryPoint = "tjDecompressHeader3")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial int DecompressHeader_x86(IntPtr handle, IntPtr jpegBuf, uint jpegSize, out int width,
        out int height, out int jpegSubsamp, out int jpegColorspace);
}
