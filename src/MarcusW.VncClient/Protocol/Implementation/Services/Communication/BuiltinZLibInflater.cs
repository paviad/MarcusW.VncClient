...
{
    private readonly Dictionary<int, Inflater> _inflaters = [];

    private volatile bool _disposed;

    /// <inheritdoc />
    public Stream ReadAndInflate(Stream source, int sourceLength, int zlibStreamId = -1,
        CancellationToken cancellationToken = default)
    {
        ObjectDisposedException.ThrowIf(_disposed, typeof(BuiltinZLibInflater));
    ...
    private sealed class Inflater : IDisposable
    {
        ...
        {
            MemoryStream = new();
            DeflateStream = new(MemoryStream, CompressionMode.Decompress, false);
        }
