...
public class RfbInitializer(RfbConnectionContext context) : IRfbInitializer
{
    private readonly ILogger<RfbInitializer> _logger = context.Connection.LoggerFactory.CreateLogger<RfbInitializer>();
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        Debug.Assert(context.Transport != null, "_context.Transport != null");
        ...
        (Size framebufferSize, PixelFormat pixelFormat, string desktopName) =
            await ReadServerInitAsync(transport, cancellationToken).ConfigureAwait(false);
        ...
        _state.RemoteFramebufferLayout = new[] { new Screen(1, new(Position.Origin, framebufferSize), 0) }
            .ToImmutableHashSet();
    ...
    private static Size GetFramebufferSize(ReadOnlySpan<byte> headerBytes)
    {
        ...
        return new(framebufferWidth, framebufferHeight);
    }
    ...
    private static PixelFormat GetPixelFormat(ReadOnlySpan<byte> headerBytes)
    {
        ...
        if (bitsPerPixel > 32)
        {
            throw new UnexpectedDataException(
                "The bits per pixel value of the received pixel format is too high. The maximum is 32bpp.");
        }
        ...
        {
            throw new UnexpectedDataException(
                "The received pixel format is invalid. The depth value must not be smaller than the sum of the used bits per channel.");
        }

        // Check the shift values
        if (redBits + redShift > bitsPerPixel || greenBits + greenShift > bitsPerPixel
            || blueBits + blueShift > bitsPerPixel)
        {
        ...
        {
            throw new UnexpectedDataException(
                "The bits of the color channels in the received pixel format must not overlap.");
        }
        ...
        return new($"RFB {name}", bitsPerPixel, depth, bigEndian, trueColor, false, redMax, greenMax, blueMax, 0,
            redShift, greenShift, blueShift, 0);
    }

    private async Task<(Size framebufferSize, PixelFormat pixelFormat, string desktopName)> ReadServerInitAsync(
        ITransport transport, CancellationToken cancellationToken = default)
    {
        ...
        ReadOnlyMemory<byte> headerBytes =
            await transport.Stream.ReadAllAsync(24, cancellationToken).ConfigureAwait(false);
        ...
        // Read desktop name
        ReadOnlyMemory<byte> desktopNameBytes =
            await transport.Stream.ReadAllAsync((int)desktopNameLength, cancellationToken).ConfigureAwait(false);
    ...
    {
        bool shared = context.Connection.Parameters.AllowSharedConnection;
