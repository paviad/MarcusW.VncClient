...
public class TransportConnector : ITransportConnector
{
    ...
    public TransportConnector(RfbConnectionContext context) : this(context.Connection.Parameters,
    ...
    private async Task<TcpTransport> ConnectTcpTransportAsync(TcpTransportParameters parameters,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await using (linkedToken.Register(() => tcpClient.Close()))
            {
                ...
                // Try to connect
                await tcpClient.ConnectAsync(parameters.Host, parameters.Port, linkedToken).ConfigureAwait(false);
            }
        }
