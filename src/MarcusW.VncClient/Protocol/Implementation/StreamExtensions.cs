...
public static class StreamExtensions
{
    ...
    public static void CopyAllTo(this Stream stream, Stream target, int numBytes,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                int read = stream.Read(bufferSpan[..Math.Min(bytesToCopy, bufferSize)]);
                if (read == 0)
                {
                    throw new UnexpectedEndOfStreamException(
                        $"Stream reached its end while trying to copy {numBytes} bytes.");
                }

                target.Write(bufferSpan[..read]);
    ...
    public static void ReadAll(this Stream stream, Span<byte> buffer, CancellationToken cancellationToken = default)
    {
        ...
        do
        {
            ...
            int read = stream.Read(bytesRead == 0 ? buffer : buffer[bytesRead..]);
            if (read == 0)
            {
                throw new UnexpectedEndOfStreamException(
    ...
    public static async Task<ReadOnlyMemory<byte>> ReadAllAsync(this Stream stream, int numBytes,
        CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        if (numBytes > 0)
        {
            ...
            do
            {
                int read = await stream.ReadAsync(buffer.AsMemory(bytesRead, numBytes - bytesRead), cancellationToken)
    ...
    public static void SkipAll(this Stream stream, int numBytes, CancellationToken cancellationToken = default)
    {
        ...
        do
        {
            ...
            // Always slice, because the rented buffer might be larger than requested.
            int read = stream.Read(bufferSpan[..Math.Min(bytesToSkip, bufferSize)]);
            if (read == 0)
            {
                throw new UnexpectedEndOfStreamException(
                    $"Stream reached its end while trying to skip {numBytes} bytes.");
            }
